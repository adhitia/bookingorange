class Admin::UsersController < ApplicationController
    ...
    # Action Edit untuk kedua tipe user (CS dan Staff)
    def edit
    end
  
    # Action Update untuk kedua tipe user
    def update
      permitted_params = if @user.customer_service?
                           params.require(:user).permit(:email, :name)
                         elsif @user.staff_cabang?
                           params.require(:user).permit(:email, :name, :branch_id)
                         else
                           params.require(:user).permit(:email, :name)
                         end
